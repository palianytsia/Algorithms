...
public class HuffmanCoder {
     ...
     * 
     * @param alphabet
     *            - map representing the alphabet to encode, where keys are the characters of the target alphabet, which
     *            can be arbitrary objects, and values are the probabilities of the character appearance in the string
     *            to encode (char frequencies); can't be null.
     * 
    ...
    public HuffmanCoder(Map<Character, Float> alphabet) {
        ...
        if (alphabet.size() == 2) {
            ...
            PriorityQueue<MetaCharacter> metaAlphabet = new PriorityQueue<MetaCharacter>(alphabet.size(),
                    new Comparator<MetaCharacter>() {
                @Override
        ...
        while (sequence.length() != 0) {
            ...
            if (!successfulDecoding) {
                throw new IllegalArgumentException(
                        "Encoding sequence is either corrupted or invalid and cannot be decoded.");
            }
        }
    ...
    public String encode(String sequence) {
        ...
        for (char c : sequence.toCharArray()) {
            ...
            if (code == null) {
                throw new IllegalArgumentException(
                        "Sequence contains a character, that does not belong to the alphabet this coder is based on: "
                                + c);
            }
            encoded.append(code);
        }
