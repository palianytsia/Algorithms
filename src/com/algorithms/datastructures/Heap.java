...
/**
 * This is a demonstration of how the heap works. Heap is a data structure that
 * offers a fast way to do repeated minimum computation for the contained
 * elements. The synonymous name for the "heap" is "priority queue".
 * 
 * <p>
 * Generally heap can contain any elements associated with the keys that are
 * comparable (i.e. applicable for minimum computation), but this demo heap can
 * contain only integers which at the same moment are the keys.
 * </p>
 * 
 * @author Ivan Palianytsia <a
 *         href="mailto:ivan.palianytsia@gmail.com">ivan.palianytsia
 *         @gmail.com</a>
 */
public class Heap {
	 ...
	 * 
	 * <p>
	 * Running time of this operation is O(log(n)), where <code>n</code> denotes
	 * the number of elements in the array, which is faster than to create an
	 * empty and then call {@link #insert(Comparable) insert} for each element
	 * in turn.
	 * </p>
	 * 
	 * @param array
	 *            - the array containing the elements to initialize the heap
	 *            with.
	 */
	...
	/**
	 * Deletes an element from the heap. If there are several such elements, an
	 * arbitrary one of them is removed.
	 * 
	 * @param element
	...
	/**
	 * Retrieves the minimum element and removes it from the heap. If there are
	 * several minimum elements, an arbitrary one of them is removed.
	 * 
	 * <p>
	 * Running time of this operation is <code>O(log(n))</code>, where
	 * <code>n</code> denotes the number of elements in the heap.
	 * </p>
	 * 
