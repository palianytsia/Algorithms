...
public class Board {
		...
		for (int actualI = 0; actualI < blocks.length; actualI++) {
			for (int actualJ = 0; actualJ < blocks.length; actualJ++) {
				if (blocks[actualI][actualJ] != 0) {
					int expectedI = (blocks[actualI][actualJ] - 1)
							/ blocks.length;
					int expectedJ = (blocks[actualI][actualJ] - 1)
							% blocks.length;
		...
		for (int actualI = 0; actualI < blocks.length; actualI++) {
			for (int actualJ = 0; actualJ < blocks.length; actualJ++) {
				if (blocks[actualI][actualJ] != 0) {
					...
					int expectedJ = (blocks[actualI][actualJ] - 1)
							% blocks.length;
					manhattan += Math.abs(actualI - expectedI)
							+ Math.abs(actualJ - expectedJ);
				}
			}
		}
		return manhattan;
	...
	public Iterable<Board> neighbors() {
		...
		if (emptyBlockIndex - blocks.length >= 0) {
			neighbours.add(swap(emptyBlockIndex - blocks.length,
					emptyBlockIndex));
		}
