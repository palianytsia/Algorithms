...
public class RandomizedQueue<Item> implements Iterable<Item> {
    ...
    /**
     * Returns an independent iterator over the items in random order. The returned iterator <strong>does not support</strong> <code>remove()</code> operation.
     * 
    ...
    /**
     * Generates a random integer from 0 (inclusive) to size of the queue (exclusive), which then can be used as an index of random item.
     * 
	...
	if (newSize < size) {
	    throw new IllegalArgumentException("New size (" + newSize + ") is not sufficient to hold all the items (" + size + ")");
	}
