 ...
 * 
 * @see <a
 *      href="http://coursera.cs.princeton.edu/algs4/assignments/queues.html">Programming
 *      Assignment 2: Randomized Queues and Deques</a> from "Algorithms, Part I"
 *      course by <i>Kevin Wayne</i> and <i>Robert Sedgewick</i> for detailed
...
public class RandomizedQueue<Item> implements Iterable<Item> {
	...
	/**
	 * The capacity (amount of space allocated) of a queue will never shrink
	 * lower this value.
	 */
	...
	public void enqueue(Item item) {
		if (item == null) {
			throw new NullPointerException(
					"Null value cannot be inserted to the queue.");
		}
	...
	/**
	 * Returns an independent iterator over the items in random order. The
	 * returned iterator <strong>does not support</strong> <code>remove()</code>
	 * operation.
	 * 
	...
	/**
	 * Generates a random integer from 0 (inclusive) to size of the queue
	 * (exclusive), which then can be used as an index of random item.
	 * 
	...
	/**
	 * Changes size of the underlying array in order to hold more items or to
	 * save space.
	 * 
	 * @param newSize
	 *            - new size for the underlying array, can't be lower than the
	 *            number of items already on this queue.
	 * @throws IllegalArgumentException
	 *             if the new physical size of the internal storage is lower
	 *             than logical size of this queue.
	 */
	private void resize(int newSize) {
		if (newSize < size) {
			throw new IllegalArgumentException("New size (" + newSize
					+ ") is not sufficient to hold all the items (" + size
					+ ")");
		}
	...
	/**
	 * Provides a way to iterate over items in a randomized queue, which
	 * internally is backed up by an array.
	 * 
	 * @author Ivan Palianytsia <a
	 *         href="mailto:ivan.palianytsia@gmail.com">ivan.palianytsia</a>
	 * 
	...
	private static class ArrayIterator<Item> implements Iterator<Item> {
		...
		/**
		 * Unsupported operation, an exception is thrown by a call to this
		 * method.
		 * 
